...
const AdminDashboard = () => {
  ...
  const { data: recentAdoptions, isLoading: adoptionsLoading } = useQuery(
    'recent-adoptions',
    async () => {
      const response = await axios.get('/adoptions?limit=5');
  ...
  const { data: recentDonations, isLoading: donationsLoading } = useQuery(
    'recent-donations',
    async () => {
      const response = await axios.get('/donations?limit=5');
  ...
  const getStatusBadge = (status) => {
    const badges = {
      ...
      accepted: 'bg-gradient-to-r from-green-400 to-emerald-500 text-white'
    };
